#include "com_dimen_ffmpegplayer_VideoPlayer.h"
#include <stdlib.h>
#include <stdio.h>
#include <unistd.h>
#include <pthread.h>
...
#define LOGI(FORMAT,...) __android_log_print(ANDROID_LOG_INFO,"dimen",FORMAT,##__VA_ARGS__);
#define LOGE(FORMAT,...) __android_log_print(ANDROID_LOG_ERROR,"dimen",FORMAT,##__VA_ARGS__);
#include "libyuv.h"
#include "queue.h"
...
void init_input_format_ctx(Player *player, const char* intput_cstr) {
	//注册组件
	av_register_all();
	...
	//打开输入视频文件
	if (avformat_open_input(&format_ctx, intput_cstr, NULL, NULL) != 0) {
		LOGE("%s", "打开输入视频文件失败");
		return;
	}
	//获取视频信息
	...
	LOGE("captrue_stream_number%d", player->captrue_stream_no);
	...
	for (i = 0; i < player->captrue_stream_no; i++) {
		...
		} else if (format_ctx->streams[i]->codec->codec_type
				== AVMEDIA_TYPE_AUDIO) {
	...
	for (;;) {
		...
		if (sleep_time < -300000ll) {
			...
			LOGI("player_wait_for_frame[%d] correcting %f to %f because late",
					stream_no, (av_gettime() - player->start_time) / 1000000.0, (av_gettime() - new_value) / 1000000.0);
...
void decode_video(Player *player, AVPacket *packet) {
	...
	AVCodecContext *codec_ctx =
			player->input_codec_ctx[player->video_stream_index];
	int got_frame;
	//解码AVPacket->AVFrame
	...
	if (got_frame) {
		...
		I420ToARGB(yuv_frame->data[0], yuv_frame->linesize[0],
				...
				rgb_frame->linesize[0], codec_ctx->width, codec_ctx->height);
 ...
 */
JNIEXPORT void JNICALL Java_com_dimen_ffmpegplayer_VideoPlayer_play(JNIEnv *env,
		jobject jobj, jstring input_jstr, jobject surface) {
	...
	//生产者线程
	pthread_create(&(player->thread_player_read_from_stream), NULL,
